...
class User():
    ...
    def inc_progress(self, kata_rank):
        if kata_rank not in self.RANKS:
            raise Exception("Error: Kata Rank Out of Bounds")
        if self.rank != 8:
            indexOfUserRank = self.RANKS.index(self.rank)
            indexOfKataRank = self.RANKS.index(kata_rank)
            differenceOfRank = indexOfKataRank - indexOfUserRank

            change = differenceOfRank ** 2 * 10 if differenceOfRank > 0 else 3 if differenceOfRank == 0 else 1 if differenceOfRank == -1 else 0
            self.progress += change
            while (self.progress >= 100):
                self.progress -= 100
                indexOfUserRank += 1
                self.rank = self.RANKS[indexOfUserRank]
                if self.rank == 8:
                    self.progress = 0