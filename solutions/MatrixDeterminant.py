...
#
# The determinant of an n x n sized matrix is calculated by reducing the problem to the calculation of the
# determinants of n matrices ofn-1 x n-1 size.
#
...
def determinant(matrix):
    ...
    for (index, num) in enumerate(matrix[0]):
        result += (1 if index % 2 == 0 else -1) * (num * determinant(reduceMatrix(copy.deepcopy(matrix), index)))
    return result
