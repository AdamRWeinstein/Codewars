...
def mix(s1, s2):
    s1 = ''.join(x for x in s1 if x in ascii_lowercase)
    s2 = ''.join(x for x in s2 if x in ascii_lowercase)
    c1 = Counter(s1)
    c2 = Counter(s2)
    c3 = c1 | c2
    x = [("=:" if c1[char] == num and c2[char] == num else "1:" if c1[char] == num else "2:", char, num) for (char, num) in c3.most_common() if num > 1]
    x.sort(key=itemgetter(1))
    x.sort(key=itemgetter(0))
    x.sort(key=itemgetter(2), reverse=True)
    return '/'.join([a + str(b * c) for (a, b, c) in x])